...
import { MdCallEnd, MdAddIcCall } from 'react-icons/md';
    ...
    <div className={styles.call_modal}>
      ...
      >
        <div className='text-center' style={{ padding: '40px 0' }}>
          <img src={call.avatar} alt='avatar' />
          <h4>{call.username}</h4>
          <h6>{call.fullname}</h6>
        ...
        <div className={styles.call_menu}>
          ...
          {call.recipient === auth.user._id && !answer && (
            <>
              {call.video ? (
                <FaVideo className={styles.calling} onClick={handleAnswer} />
              ) : (
                <MdAddIcCall
      ...
      <div
        className={styles.show_video}
        style={{
          opacity: answer && call.video ? '1' : '0',
        }}
      >
        <video ref={youVideo} className={styles.you_video} playsInline muted />
        <video ref={otherVideo} className={styles.other_video} playsInline />

        <div className={styles.time_video}>
          <span>{hours.toString().length < 2 ? '0' + hours : hours}</span>
          <span>:</span>
          <span>{mins.toString().length < 2 ? '0' + mins : mins}</span>
          <span>:</span>
        ...
        <MdCallEnd className={styles.end_call} onClick={handleEndCall} />
      </div>
    </div>
  );
};
